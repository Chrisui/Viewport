...
/**
 * Convert a bounds object in plot space to view space
 * @param {Bounds} bounds Bounds in plot space
 * @returns {Bounds}
 */
Viewport.prototype.plotToViewSpaceBounds = function(bounds) {
	return new Bounds(
		this.plotToViewSpace(bounds.pos),
		this.plotToViewSpace(bounds.size, true)
	);
};

/**
 * Convert a bounds object in plot space to client space
 * @param {Bounds} bounds Bounds in plot space
 * @returns {Bounds}
 */
Viewport.prototype.plotToClientSpaceBounds = function(bounds) {
	return new Bounds(
		this.plotToClientSpace(bounds.pos),
		this.plotToClientSpace(bounds.size, true)
	);
};

/**
 * Convert a bounds object in view space to plot space
 * @param {Bounds} bounds Bounds in view space
 * @returns {Bounds}
 */
Viewport.prototype.viewToPlotSpaceBounds = function(bounds) {
	return new Bounds(
		this.viewToPlotSpace(bounds.pos),
		this.viewToPlotSpace(bounds.size, true)
	);
};

/**
 * Convert a bounds object in view space to client space
 * @param {Bounds} bounds Bounds in view space
 * @returns {Bounds}
 */
Viewport.prototype.viewToClientSpaceBounds = function(bounds) {
	return new Bounds(
		this.viewToClientSpace(bounds.pos),
		bounds.size.clone()
	);
};

/**
 * Convert a bounds object in client space to view space
 * @param {Bounds} bounds Bounds in client space
 * @returns {Bounds}
 */
Viewport.prototype.clientToViewSpaceBounds = function(bounds) {
	return new Bounds(
		this.clientToViewSpace(bounds.pos),
		bounds.size.clone()
	);
};

/**
 * Convert a bounds object in client space to plot space
 * @param {Bounds} bounds Bounds in client space
 * @returns {Bounds}
 */
Viewport.prototype.clientToPlotSpaceBounds = function(bounds) {
	return new Bounds(
		this.clientToPlotSpace(bounds.pos),
		this.clientToPlotSpace(bounds.size, true)
	);
};
...
Viewport.prototype.zoom = Viewport.prototype.zoomPlotSpace = function(basicDelta, zoomCoordinate) {
    ...
    zoomCoordinate = zoomCoordinate || this.size.clone().multiply(new Victor(0.5, 0.5)).add(this.pos);

    var normalisedZoomPoint = zoomCoordinate.clone()
        .subtract(this.pos)
        .divide(curRangeDistance);

    // Calculate the new zoomed range distance
	var multiplier = ZOOM_MULTIPLIER * Math.abs(basicDelta);
    var newRangeDistance = curRangeDistance.clone()[basicDelta > 0 ? 'divide' : 'multiply'](new Victor(multiplier, multiplier));
    ...
    this.pos.copy(
        zoomCoordinate.clone()
            .subtract(
                newRangeDistance.multiply(normalisedZoomPoint)
            )
    );
